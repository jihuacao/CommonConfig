...
docker pull registry.cn-guangzhou.aliyuncs.com/payforsins/xray:latest
# todo: 这里需要增加判断
...
docker run -it --ipc host --network CI --network-alias xray --name xray -v ${start_bash_dir}:/etc/xray-kcptun-client/ -v ${xray_dir}:${xray_dir} -v ${kcptun_dir}:${kcptun_dir} -t registry.cn-guangzhou.aliyuncs.com/payforsins/xray:latest /bin/bash -c "chmod +x /etc/xray-kcptun-client/start_xray-kcptun_client.sh && /etc/xray-kcptun-client/start_xray-kcptun_client.sh"
